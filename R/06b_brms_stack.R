...
# A cached OOF file older than the tuned model was generated by a previous
# model fit -- calibrating the stack against it silently mismatches production.
oof_stale <- file.exists(oof_cache) && file.mtime(oof_cache) < file.mtime(model_file)
if (oof_stale) {
  cli_alert_warning("OOF cache predates {basename(model_file)} -- regenerating")
}

if (file.exists(oof_cache) && !oof_stale) {
...
# NOTE (2026-07-02): do NOT add form features (e.g. form_residual_mean_4) as
# fixed effects here. Tried and reverted same day: alongside (1 | player_id)
# the coefficient estimates the WITHIN-player effect -- mean reversion, fitted
# -0.06 -- which pulls currently-hot players DOWN at prediction time, the
# opposite of the intended calibration. Marginally (no RE, gbdt_pred slope
